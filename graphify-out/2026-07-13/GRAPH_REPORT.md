...
## Corpus Check
- 68 files · ~1,500,287 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 738 nodes · 1495 edges · 41 communities (36 shown, 5 thin omitted)
...
Cohesion: 0.07
Nodes (55): PageClassification, PageClassifier, Page classifier — determines page number (1-6) from image content.  Uses Tessera, Classify a page from an image file., Classify all images and return a mapping of image_index -> classification., Resolve page ordering from classifications.          Returns:           - mappin, Classifies page images into page numbers 1-6 based on OCR content., Classify a single page from its OCR text content. (+47 more)

### Community 4 - "pipeline_runner.py"
Cohesion: 0.05
Nodes (87): FastAPI, Response, main(), close_pool(), create_job(), Close the connection pool. Call once at server shutdown., Upsert a row into ocr_documents keyed on job_id.     Structured field columns ar, Insert a new job with status='submitted'. (+79 more)

### Community 5 - "ExtractionPipeline"
Cohesion: 0.11
Nodes (17): adaptive_threshold(), denoise(), deskew(), ExtractionPipeline, _load_page_images_cv(), preprocess(), ndarray, Preprocess image files (deskew, optional color, downscale). (+9 more)

### Community 6 - "ChandraOcrClient"
Cohesion: 0.10
Nodes (22): ChandraOcrClient, _clean_value(), _extract_inline_value(), _extract_table_from_html(), _get_checkbox_suffixes(), _is_all_options_line(), _is_option_items_line(), _is_placeholder() (+14 more)
...
Cohesion: 0.16
Nodes (21): _classify_checkbox_mark(), _crop_checkbox(), _detect_both_marked(), load_page_images(), _normalize_mark(), _points_to_pixels(), ndarray, Path (+13 more)
...
Cohesion: 0.19
Nodes (15): _get_bbox_cache_key(), _get_tesseract_semaphore(), _load_bbox_cache(), OCRBackend, OCRResult, ABC, ndarray, Path (+7 more)
...
Cohesion: 0.10
Nodes (25): Pool, _extract_structured_fields(), get_incomplete_jobs(), get_last_job_id_by_pdf_id(), get_pool(), get_result_by_file_hash(), get_result_by_job_id(), get_stuck_jobs() (+17 more)

### Community 38 - "run_batch_pdfs_pipeline_async"
Cohesion: 0.25
Nodes (4): ModelClient, ABC, Optional: submit PDF async and return a job reference immediately.         Overr, Optional: poll a submitted job until complete.         Override in clients that
...
Cohesion: 0.20
Nodes (6): Any, RolmOCR + GPT-4o-mini extraction client.  Two-stage pipeline:   Stage 1: RolmOCR, RolmOcrClient, compute_sections(), Shared section definitions derived from KNOWN_TEMPLATE_FIELDS.  Avoids hardcodin, Compute sections list dynamically from KNOWN_TEMPLATE_FIELDS.
...
- **Why does `TokenUsage` connect `model_client.py` to `RolmOcrClient`, `DocumentReview.tsx`, `ChandraOcrClient`?**
  _High betweenness centrality (0.215) - this node is a cross-community bridge._
- **Why does `convert_extract_response()` connect `datalab_schema.py` to `model_client.py`, `pipeline_runner.py`, `RolmOcrClient`?**
  _High betweenness centrality (0.120) - this node is a cross-community bridge._
- **Why does `DatalabOcrClient` connect `model_client.py` to `model_client.py`, `pipeline_runner.py`, `run_batch_pdfs_pipeline_async`, `ExtractionPipeline`, `datalab_schema.py`, `OpenAICompatibleClient`?**
